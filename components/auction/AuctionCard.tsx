...
export default function AuctionCard({
  ...
  return (
    <Link
      href={`/auction/${id}`}
      className="flex min-w-[300px] flex-col overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900"
    >
      <div className="relative h-44 w-full overflow-hidden bg-coffee-cream/20">
        <div
          className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
          style={{ backgroundImage: `url('${getImageWithFallback(image)}')` }}
        ></div>
