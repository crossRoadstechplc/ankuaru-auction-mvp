...
export function AuctionDetailsCard({ data }: AuctionDetailsCardProps) {
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
      <div className="aspect-video relative overflow-hidden group">
        ...
        />
        <div className={`absolute top-4 left-4 ${data.status === "CLOSED" ? "bg-slate-500" : "bg-primary"} text-white px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase`}>
      ...
      <div className="p-8">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4 pb-6 border-b border-slate-100 dark:border-slate-800 mb-6 font-display">
              ...
              <div className="flex flex-col">
                ...
                <span className={`text-lg font-bold ${data.status === "CLOSED" ? "text-slate-400" : "text-red-500"}`}>
                  {data.status === "CLOSED" ? "Ended" : data.timeLeft}
                </span>
              </div>
            </div>
          ...
          <div className="text-right">
            ...
            <p className="text-slate-900 dark:text-white font-mono font-bold">ANK-{data.id || "000"}</p>
          </div>
        </div>
