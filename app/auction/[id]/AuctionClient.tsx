...
export default function AuctionClient({ initialAuction, id }: AuctionClientProps) {
  const searchParams = useSearchParams();
  const isCreator = searchParams.get("view") === "creator";
  const { isAuthenticated, user } = useAuthStore();

  // React Query hooks with initial data from SSR
  const {
    data: auction,
    isLoading: auctionLoading,
    error: auctionError,
  } = useAuction(id, initialAuction, (isCreator || !!user?.id) ? 5000 : false);

  const isOwner = isCreator || (!!user?.id && user?.id === auction?.createdBy);
  const pollInterval = (isOwner && auction?.status !== "CLOSED") ? 5000 : false;

  const { data: bids = [] } = useAuctionBids(id, pollInterval);

  // Use centralized hook for creator info
  const { data: creatorInfo, isLoading: creatorLoading } = useUser(auction?.createdBy || "");

  if (!isAuthenticated && !initialAuction) {
     // If not authenticated and no SSR data (likely private or error), show login
    return (
  ...
  if ((creatorLoading || auctionLoading) && !auction) {
    return (
  ...
  if ((auctionError || !auction) && !auctionLoading) {
    return (
      <div className="dark:bg-background-dark text-slate-900 dark:text-slate-100 antialiased bg-[#f8f5f0] min-h-screen">
        <Header />
        <main className="flex-1 max-w-7xl mx-auto w-full px-4 lg:px-8 py-8">
